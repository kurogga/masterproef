...
public class LightField_Flowers : MonoBehaviour
{
    public string path = "Assets/Resources/Flowers_fixed.csv";
    public TextAsset textFile;
    public Material material;
    public Vector2 mousePosition;
    public const int KERNELS = 9537;
    ...
    void Start()
    {
        ...
        for (int i = 0; i < eachLine.Count - 1; i++)
        {
            ...
            float rvalue = Convert.ToSingle(nrs[5])/100.0f;
            float gvalue = Convert.ToSingle(nrs[6])/100.0f;
            float bvalue = Convert.ToSingle(nrs[7])/100.0f;
    ...
    void Update()
    {
        mousePosition = new Vector2(Input.mousePosition.x/Screen.width*30,Input.mousePosition.y/Screen.height*30);

...
public class LightFieldBlocksWColorGrad : MonoBehaviour
{
    ...
    int totalMinBlocks;
    int withGradient;
    public const int PIXELS_PER_BLOCK = 64;
...
#endif
        mousePosition.x = Mathf.Clamp(mousePosition.x, 5f, 10f);
        mousePosition.y = Mathf.Clamp(mousePosition.y, 5f, 10f);
    ...
    void CalculateBestKernelsAsync2(Vector4 xVector, int currentBlockSize)
    {
        ...
        for (int i = 0; i < givenKernelsAmount; i++)
        {
            ...
            float lowerBoundY = midPixelY - halfLengthY * halfLengthY;
            float upperBoundY = midPixelY + halfLengthY * halfLengthY;
        ...
        if (relKernelsCount <= 8 || currentBlockSize == MIN_PIXELS_PER_BLOCK)
        {
            ...
            if (currentBlockSize == MIN_PIXELS_PER_BLOCK)
            {
                ...
                // UnityEngine.Debug.Log(startBlock + ": " + bufferIndex + " min");
                for (int i = 0; i < KERNELS_PER_BLOCK; i++)
                {
                    ...
                    gradientList[bufferIndex + i] = newKernelList[i].Gradient;
                ...
                while (k < toFillRows && startBlock < totalMinBlocks)
                {
                    ...
                    while (l < toFillRows && bufferIndex < totalMinBlocks * KERNELS_PER_BLOCK)
                    {
                        ...
                        for (int index = 0; index < KERNELS_PER_BLOCK; index++)
                        {
                            ...
                            gradientList[bufferIndex + index] = newKernelList[index].Gradient;
    ...
    void InitializeVariables()
    {
        ...
        UnityEngine.Debug.Log("totaal:" + (totalMinBlocks * KERNELS_PER_BLOCK));
        ...
        for (int i = 0; i < eachLine.Count - 1; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                for (int k = 0; k < 4; k++)
                {
                    ...
                    gradient[j + (k * 4)] = matrixValue / 500f;
                }
                // gradient[j] /= 2f;
                // gradient[j + 4] /= 2f;
            }

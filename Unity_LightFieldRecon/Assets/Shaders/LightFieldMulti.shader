	...
	SubShader
	{
		...
		Pass
		{
            ...
            float4 frag (v2f v) : SV_Target
            {
				...
				// int amountBlockY = floor(frameHeight/pixelsPerBlock);
				// blockY = amountBlockY - blockY;
				...
				if(blockX != 0)
				{
					...
					for (int i = startBlock*kernelsPerBlock-kernelsPerBlock; i < (startBlock*kernelsPerBlock); i++)
					{
						weight = muYnPiList[i].w*getNormalDist(x, i);
						weightSum += weight;
						// Calculate weight * muY(color)
				...
				if(blockX != amountBlocksInARow)
				{
					...
					for (int i = startBlock*kernelsPerBlock+kernelsPerBlock; i < (startBlock*kernelsPerBlock)+2*kernelsPerBlock; i++)
					{
						weight = muYnPiList[i].w*getNormalDist(x, i);
						weightSum += weight;
						// Calculate weight * muY(color)
				...
				if(blockY != amountBlocksInAColumn)
				{
					...
					for (int i = startBlock*kernelsPerBlock+amountBlocksInARow*kernelsPerBlock; i < (startBlock*kernelsPerBlock)+kernelsPerBlock+amountBlocksInARow*kernelsPerBlock; i++)
					{
						weight = muYnPiList[i].w*getNormalDist(x, i);
						weightSum += weight;
						// Calculate weight * muY(color)

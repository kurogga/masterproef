Shader "Unlit/LightField2b"
{
	SubShader
	{
		...
		Pass
		{
			...
			uniform StructuredBuffer<float4x4> coMatrixInvList; // coMatrix^(-1) * -1/2
            ...
            float4 frag (v2f v) : SV_Target
            {
                ...
                float4 x = float4(mouseX,mouseY, v.vertex.x/_ScreenParams.x, 1-v.vertex.y/_ScreenParams.y);
				...
				{
					float weight = muYnPiList[i].w*getNormalDist(x, i);
					weightSum += weight;
